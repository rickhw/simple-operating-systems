 ...
 * @brief 分頁管理子系統 (Paging & Virtual Memory Management)
 *
 * ==============================================================================
 * [圖 1] x86 32-bit 虛擬位址轉換機制 (Virtual Address Translation)
 * ==============================================================================
 * * 32-bit Virtual Address (虛擬位址)
 * +--------------------+--------------------+----------------------+
 ...
 * (實體位址)            | (1024 entries|     v
 * +--------------+     +--------------+     +--------------+
 * | Page Dir     |     | [0] PTE      |     | Physical Page|
 * | (1024 entries|     | ...          |     | (4KB Size)   |
 * +--------------+     | [Table Index]|---->|              |
 * | [0] PDE      |     | ...          |     | + Offset     |---> 最終實體位址
 * | ...          |     | [1023] PTE   |     |              |     (Physical Address)
 * | [Dir Index]  |---> +--------------+     +--------------+
 * | ...          |     (實體位址)
 * | [1023] PDE   |
 * +--------------+
 * * ==============================================================================
 * [圖 2] 單一行程虛擬記憶體空間佈局 (Per-Process Virtual Memory Layout)
 * ==============================================================================
 * * 高位址 (High Memory)
 * 0xFFFFFFFF +-----------------------+
 * |                                  |
 * |    Kernel Space (核心共享區段)     | (通常包含 Kernel Heap, PMM, VFS 等)
 * |    (所有行程共用相同的分頁映射)      |
 * |                                  |
 * 0xC0000000 +-----------------------+ <-- KERNEL_BASE (依系統設計而定)
 * |                                  |
 * |    Unmapped / Reserved           |
 * |                                  |
 * 0x10000000 +-----------------------+ <-- USER_HEAP_START
 * |    User Heap (動態配置區)          | (預先分配 256 Pages = 1MB)
 * |    |                             |
 * |    v (向下增長)                   |
 ...
 * |    Unmapped / 保留給未來擴充        |
 * |                                  |
 * 0x083FF000 +-----------------------+ <-- USER_STACK_TOP
 * |    ^ (向上增長)                   |
 * |    |                             |
 * |    User Stack (使用者堆疊)        | (初始分配 1 Page = 4KB)
 * 0x083FE000 +-----------------------+
 * |                                  |
 * |    User Code / Data (ELF 載入區)  | <-- ELF Entry Point 通常落在此區間
 * |                                  |
 * 0x00000000 +-----------------------+
 * 低位址 (Low Memory)
 *
