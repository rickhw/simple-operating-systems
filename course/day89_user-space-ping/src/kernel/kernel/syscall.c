 ...
 *
 * This file contains the system call dispatcher and handlers for various
 * subsystems including file system, task management, IPC, and GUI.
 */
...
#define MAX_MESSAGES 16 // IPC Message Queue
#define SYS_SET_DISPLAY_MODE 99 // Switch between desktop and terminal mode.
...
static void handle_task_syscalls(uint32_t eax, registers_t *regs) {
    ...
    else if (eax == 24) regs->eax = sys_kill((uint32_t)regs->ebx);
}
...
void syscall_handler(registers_t *regs) {
    ...
    if (eax == 25) { // get_mem_info
    ...
    else if (eax == 29) { // sys_get_window_event
        ...
        // Security check: ensure window exists and is owned by the current process
        if (win != 0 && win->owner_pid == current_task->pid && win->has_new_click) {
            *out_x = win->last_click_x;
            *out_y = win->last_click_y;
            win->has_new_click = 0; // Event read, lower the flag
            regs->eax = 1; // Return 1 for new event
        } else {
            regs->eax = 0; // Return 0 for no event
        }
        ipc_unlock();
    }
    else if (eax == 30) { // sys_get_window_key_event
        int win_id = (int)regs->ebx;
        ...
        if (win != 0 && win->owner_pid == current_task->pid && win->has_new_key) {
            *out_key = win->last_key;
            win->has_new_key = 0; // Event read, lower the flag
            regs->eax = 1;        // New key event
        } else {
            regs->eax = 0;        // No key event
        }
        ipc_unlock();
    }

    // ==========================================
    // 【新增】31 號 Syscall：網路 Ping 發送
    // ==========================================
    else if (eax == 31) {
        uint8_t* target_ip = (uint8_t*)regs->ebx;

        // 由於發送封包可能會有一點點等待時間，我們不鎖住整個 IPC
        // 直接呼叫網路層的 Ping 發送器
