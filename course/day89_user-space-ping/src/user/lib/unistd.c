...
int create_file(const char* filename, const char* content) { return syscall(14, (int)filename, (int)content, 0, 0, 0); }

int remove(const char* filename) { return syscall(16, (int)filename, 0, 0, 0, 0); }

// 檔案 I/O 操作
int open(const char* filename) { return syscall(3, (int)filename, 0, 0, 0, 0); }

int read(int fd, void* buffer, int size) { return syscall(4, fd, (int)buffer, size, 0, 0); }

// directory
int readdir(int index, char* out_name, int* out_size, int* out_type) { return syscall(15, index, (int)out_name, (int)out_size, (int)out_type, 0); }

int mkdir(const char* filename) { return syscall(17, (int)filename, 0, 0, 0, 0); }

// change dir: cd
int chdir(const char* dirname) { return syscall(18, (int)dirname, 0, 0, 0, 0); }

// current working directory
int getcwd(char* buffer) { return syscall(19, (int)buffer, 0, 0, 0, 0); }
