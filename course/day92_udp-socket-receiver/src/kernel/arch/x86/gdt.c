...
extern void gdt_flush(uint32_t);    // 外部組合語言函式，用來載入 Global Descriptor Table (Global Descriptor Table)
extern void tss_flush(void);        // 外部組合語言函式，用來載入 TSS (Task State Segment)

// 設定單一 Global Descriptor Table 條目的輔助函式
...
static void write_tss(int32_t num, uint16_t ss0, uint32_t esp0) {
    ...
    // 把 TSS 當作一個特殊的 Global Descriptor Table 區段加進去 (Access: 0xE9 代表 TSS)
...
void init_gdt(void) {
    ...
    // 呼叫組合語言，正式套用新的 Global Descriptor Table
...
void set_kernel_stack(uint32_t esp) {
    // 0x10 代表 Kernel Data Segment (你的 Global Descriptor Table 設定中，Data 段的 offset)
    tss_entry.ss0 = 0x10;
    tss_entry.esp0 = esp;
}
